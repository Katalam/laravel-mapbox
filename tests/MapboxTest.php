...
describe('request api', function () {
    it('can get coordinate for postal code', function () {
        ...
        $response = Mapbox::getCoordinateForPostalCode('10115');

        expect($response)->toBeArray();
    });
});

describe('map geo json feature collection to coordinates', function () {
    it('works', function () {
        $data = [
            [
                'type' => 'Feature',
        ...
        $response = Mapbox::mapGeoJsonFeatureCollectionToCoordinates($data);

        expect($response)->toBeInstanceOf(Collection::class)
            ->and($response->count())->toBe(1)
            ->and($response->first())->toBeInstanceOf(Coordinate::class)
            ->and($response->first()->getValue()->getLatitude())->toBe(52.527920)
            ->and($response->first()->getValue()->getLongitude())->toBe(13.386345);
    });
});
